...
<?php if ($current_user['roles'][0] == 'elite_usa_insurance_agent') : ?>
      <v-dialog v-model="edit_dialog" max-width="75%" style="z-index: 100000;">
        <v-card>
          ...
          <v-card-text>
            <v-container fluid>
              <v-row v-if="inbox.editedIndex != -1" style="display: inline">
                <v-form ref="request_edit_form" v-model="request_edit_form_valid" lazy-validation>
                 <v-col cols="12">
                   <v-textarea v-model="modifications.content" label="Specify the modifications to do" :rules="rules.required" auto-grow outlined></v-textarea>
                 </v-col>
                </v-form>
              </v-row>
