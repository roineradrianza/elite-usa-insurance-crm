  ...
  <v-dialog v-model="view_dialog" max-width="95%" style="z-index: 100000;" @click:outside="closeView">
    <v-card>
      ...
      <v-card-text>
        <v-container fluid>
          <v-row ref="preview_quote" v-if="inbox.editedIndex != -1">
            <v-col class="d-flex justify-end mt-n10" cols="12">
              ...
              <?php endif ?>
              <v-chip :color="checkStatusColor(inbox.editedItem.status, inbox.editedItem.post_type)">{{ returnStatusType(inbox.editedItem.status, inbox.editedItem.post_type) }}</v-chip>
            </v-col>
