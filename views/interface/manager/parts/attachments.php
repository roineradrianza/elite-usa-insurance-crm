...
<v-col cols="12" md="6">
    <v-data-table :headers="attachments.header" :items="attachments.items" item-key="ID"
        sort-by="['status','published_at']" class="elevation-1" :loading="attachments_table_loading" multi-sort>
        <template #top>
                        ...
                        <v-card-text>
                            <v-container class="mt-n10">
                                <v-form>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field label="Document to be requested"
                                                v-model="attachments.editedItem.post_title"></v-text-field>
                                        </v-col>
                                    </v-row>
                                </v-form>
                            </v-container>
                                    ...
                                    <v-row>
                                        <v-col cols="12">
                                            ...
                                            <v-file-input v-model="attachments.editedItem.doc"
                                                ...
                                                label="Upload Document" multiple counter show-size filled
        ...
        <template #item.post_title="props">
            <v-edit-dialog ref="attachment_edit_dialog" @save="updateAttachmentName(props.item)"
                :return-value.sync="props.item.post_title" large>
                {{ props.item.post_title }} <v-icon color="success" small>mdi-pencil</v-icon>
                <template #input>
                    <v-text-field v-model="props.item.post_title" label="Edit Document"
                        hint="Write the new document name" single-line autofocus></v-text-field>
                </template>
            </v-edit-dialog>
        </template>
        <?php endif?>
        <template #item.actions="props">
            <v-row class="d-flex justify-center pr-4" dense>
                ...
                <template v-if="parseInt(props.item.status) == 2">
                    <v-tooltip top>
                        <template #activator="{ on, attrs }">
                            <v-icon color="warning" @click="markAsProcessingDocumentRequested(props.item)"
                ...
                <template v-else-if="parseInt(props.item.status) == 3">
                    <v-tooltip top>
                        <template #activator="{ on, attrs }">
                            <v-icon color="success" @click="approveDocumentRequested(props.item)" v-bind="attrs"
                                v-on="on">
                                mdi-check-circle
                            </v-icon>
                        </template>
                ...
                <template v-if="props.item.attachment_url != ''">
                    <template v-if="typeof props.item.attachment_url === 'string'">
                        <v-btn :href="props.item.attachment_url" download icon text>
                            <v-icon color="primary">mdi-file-download</v-icon>
                        </v-btn>
                    </template>
                    <template v-else>
                        <v-btn @click="download(props.item.attachment_url)" icon text>
                            <v-icon color="primary">mdi-file-download</v-icon>
                        </v-btn>
                    </template>
                </template>
                ...
                <?php if ($current_user['roles'][0] == 'elite_usa_superuser'): ?>
                <v-btn class="ml-n2 mr-n1" @click="editUploadAttachmentItem(props.item)" icon text>
                ...
                <v-btn @click="editAttachmentItem(props.item)" icon text>
                    <v-icon color="primary">mdi-upload</v-icon>
                </v-btn>
                <?php endif?>
            </v-row>
