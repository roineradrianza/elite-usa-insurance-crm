        ...
        <v-main>
            ...
            <v-container class="mb-n2" fluid>
                <v-row>
                    ...
                    <v-col cols="10">
                        <?php echo \RA_ELITE_USA\Controller\Classes\Template::show_template('interface/manager/parts/inbox/preview') ?>
                        <?php echo \RA_ELITE_USA\Controller\Classes\Template::show_template('interface/manager/parts/inbox/edit') ?>
                        <?php echo \RA_ELITE_USA\Controller\Classes\Template::show_template('interface/manager/parts/inbox/delete') ?>
                        ...
                        <v-row>
                            <v-col cols="12">
                                <v-data-table :headers="inbox.header" :items="inbox.items"
                                    sort-by="['status','published_at']" :search="inbox.search" class="elevation-1"
                                    :loading="table_loading" multi-sort>
                                    <template #top>
                                        <v-row class="px-10 d-flex align-center">
                                            <v-col cols="11">
                                                <v-text-field v-model="inbox.search" append-icon="mdi-magnify"
                                                    label="Search"
                                                    placeholder="search by quote id, date, type, description or status..."
                                                    hint="search by quote id, date, type, description or status..."
                                    ...
                                    <template #item.post_type="{ item }">
                                        <template v-if="item.post_type == 'quote_form' && item.post_parent != 0">
                                          Quote Form Renewal
                                        </template>
                                        <template v-else>
                                    ...
                                    <template #item.actions="{ item }">
                                        ...
                                        <v-icon class="ml-n2" md color="primary" @click="showDetailsItem(item)"
                                            v-if="item.post_type == 'quote_form_mr' || item.post_type == 'quote_data_r'">
                                            mdi-eye
                                        </v-icon>
                                    </template>
                                    <template #item.status="{ item }">
                                        <v-chip :color="checkStatusColor(item.status, item.post_type)" dark>
                                            {{ returnStatusType(item.status, item.post_type) }}</v-chip>
                                    </template>
                                </v-data-table>
                            </v-col>
                        </v-row>
